...
- JSON 数据备份与恢复
- 支持添加到安卓主屏幕和离线访问
- 安卓应用内登录综合教务系统，下载后自动解析课表 PDF

## 数据与隐私

课程、备注、调课和停课记录默认只保存在当前浏览器的 `localStorage` 中，不会上传账号密码或课表数据。清除浏览器数据或更换设备前，请先使用“备份与恢复”导出 JSON 文件。

仓库内置的初始课表和项目截图均为虚构演示数据，不包含作者的真实课程、教师、教室或课程编号。

安卓端的账号、密码和验证码只在大连理工大学官方登录页面中填写。应用不会读取或保存密码；导入窗口关闭时会清除登录 Cookie，下载到应用私有目录的 PDF 只用于本地解析。
...
在应用内点击“教务系统导入”，完成统一身份认证并进入“我的课表”，再点击“查找并导入课表”。应用会捕获教务系统导出的 PDF，并交给现有解析器处理；原来的手动 PDF 导入入口仍然保留。
...
项目的线上版本运行在 Cloudflare Workers 兼容环境中。
