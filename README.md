...
1. 打开仓库的 **Actions → Verify and publish Android APK**。
2. 点击 **Run workflow**。
3. 构建完成后下载 `tianyang-schedule-release`。
4. 解压并安装其中的 `天扬课表-v*.apk`。

维护者可按照 [Android 正式版发布说明](docs/RELEASE.md)配置签名并自动创建 GitHub Release。正式签名密钥不会保存在公开仓库中。
...
教务网页读取以课程名称、时间、周次和地点为准。网页导入不再尝试通过悬浮层抓取教师信息；课程卡片不会显示“待定”，详情页会标记为“未获取”。从本地文件成功解析到的教师信息仍会正常保留。
