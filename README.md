...
在应用内点击“教务系统导入”，应用会打开综合教务系统首页 `http://jxgl.dlut.edu.cn/student/home`。完成统一身份认证后点一次“自动查找并导入”，应用会自动进入“我的课表”；网页出现完整课程卡片后会直接读取课程数据并导入，不需要再展开“导出”菜单。网页结构无法识别时，应用才继续尝试学校 PDF 导出流程；原来的手动 PDF 导入入口仍然保留。
